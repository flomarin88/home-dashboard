...
interface UndoState {
  ...
  offer: (label: string, undo: () => void, dwellMs?: number, now?: number) => number
...
export const useUndoStore = create<UndoState>((set, get) => ({
  ...
  offer: (label, undo, dwellMs = 7000, now = Date.now()) => {
    const id = nextId++
    set({ current: { id, label, undo, offeredAt: now, expiresAt: now + dwellMs } })
    return id
  },
  dismiss: (id) =>
    set((s) => {
 ...
 */
export function offerUndo(label: string, undo: () => void, dwellMs?: number): number {
