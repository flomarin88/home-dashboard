...
vi.mock('@hakit/core', () => ({
  useEntity: (id: string) => {
    ...
    if (id.includes('carbone'))
      return { state: state.co2, last_changed, attributes: { unit_of_measurement: 'ppm' } }
    if (id.includes('noise'))
      return { state: state.noise, last_changed, attributes: { unit_of_measurement: 'dB' } }
    ...
    <MemoryRouter initialEntries={['/room/salon']}>
      <Routes>
        <Route
          path="/room/salon"
          element={
            <RoomDetailContent room={{ id: 'salon', label: 'Salon', kid: false }} />
          }
        />
...
describe('RoomDetail (Story 6.2 → room history)', () => {
  it('renders a chart per mapped sensor with current values (Salon has the sonomètre)', async () => {
    ...
    expect(await screen.findAllByRole('img', { name: /Historique/i })).toHaveLength(4)
  })

  it('colours the CO₂ current value by air-quality threshold (620 → green)', () => {
    renderPage()
    expect(screen.getByText(/620\s*ppm/).className).toContain('text-security-ok')
  })
