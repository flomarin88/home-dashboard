...
vi.mock("@hakit/core", () => ({
  useEntity: (id: string) => {
    if (id.includes("map"))
      return {
        attributes: { entity_picture: "/api/image_proxy/map?token=REDACTED" },
      };
    ...
    if (id.includes("temps_restant"))
      return { state: "665461", attributes: {} }; // ~8 j
    ...
    if (id.includes("duree"))
      return { state: "3", attributes: { unit_of_measurement: "s" } };
    return {
...
describe("VacuumDetail (Story 5.3)", () => {
  ...
  it("renders the curated content as tiles (no section headings)", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Quotidien" }),
    ).toBeInTheDocument();
    expect(
