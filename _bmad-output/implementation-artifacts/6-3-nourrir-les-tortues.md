...
- **Layout (retour Florian, preuve device)** : `TopBarSlots` repassé de **cluster centré → ancré à gauche (`left-44`)** pour coller la météo à l'heure ; ordre des enfants **météo → tortue → poubelle** (commit `b0cab0e`). _(Ajustement de 6.4, voir sa Change Log.)_
...
| --- | --- | --- |
| 2026-07-17 | 0.3 | **Revue multi-agent (Blind + Edge + Acceptance).** Corrigé : **P1** garde anti-double-tap dans `TurtleTile` (le compteur n'est **pas** idempotent → un double-tap faisait 0→2 ; `useRef` in-flight libéré à l'écho HA — write-debounce, pas d'optimiste d'affichage) ; **D1** toutes les tuiles top-bar (tortue, poubelle, météo, + boutons `TopBar`) passées **48→56px** (AC ≥56px, NFR2) ; **P2** ordre corrigé dans les change logs. +1 test (double-tap), 146 verts. Déférés (`deferred-work.md`) : durcissement collision top-bar, distinction visuelle/aria « fait » vs hors-ligne. ⚠️ le passage à 56px des boutons `TopBar` (en flux) est à vérifier au device-proof (contrainte sans-scroll). |
