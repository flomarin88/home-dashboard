...
- [x] **Task 10 — Bouton « Aujourd'hui »** (demande Florian 2026-07-29, après la Task 9) — **TDD**
  - [x] À **droite des flèches**, dans la même rangée de contrôle de 52px, cible **≥ 44px**. Ramène l'ancrage à la date réelle, donc à la période **contenant** aujourd'hui — le jour, la semaine ou le mois selon la vue active.
  - [x] **Toujours actif, jamais `disabled`** : un contrôle désactivé est une cible morte sur un kiosque, et les lecteurs d'écran le sautent (précédent consigné en revue de 6.1). Un clic alors qu'on y est déjà est **idempotent** — même ancrage, même plage, aucune requête.
  - [x] Tests : retour depuis chaque vue après navigation ; idempotence ; cible ≥44px ; le repère « aujourd'hui » réapparaît.
...
- [x] **Task 8 — Validation (gates)** (AC: 8)
  ...
  - [x] **Preuve device (Florian)** : les trois vues à 1024×748 **sans scroll**, un jour chargé (récurrents « Enfants ») **et** pendant un multi-jours en cours ; retour à l'accueil ; HA coupé ⇒ dernière réponse + atténuation. — _en attente Florian_ — ✅ **VALIDÉE PAR FLORIAN LE 2026-07-29 sur l'iPad du kiosque**, navigation et bouton « Aujourd'hui » compris. Inclut l'invariant sans filet automatisé : les trois vues à 1024×748 **sans scroll** (TD-9).
...
- **🚨 INCIDENT — 8 déploiements en échec, prod bloquée 11 commits en arrière (2026-07-28 → 29).** Le commit `a39a1a8` a inscrit la vraie réponse HA en fixture, dont `"2026-07-29T23:45:00+02:00"` — un **instant absolu**. `formatEventTime` le rend en heure **locale** : « 23:45 » à Paris, « **21:45** » sur un runner en UTC. Le test passait donc chez le dev et échouait en CI, à chaque push, sans que rien de visible ne le signale : Florian a découvert le blocage en constatant que sa prod était restée sur `feeb498`. **Le pre-commit ne pouvait pas l'attraper** — il tourne dans le fuseau du dev. **Correctif à la racine** : `TZ: "Europe/Paris"` épinglé dans `vitest.config.ts`, le fuseau de l'appareil cible. Suite revérifiée verte sous **UTC, America/New_York et Asia/Tokyo**. _Leçon : une fixture reprise du réel apporte ses instants absolus avec elle ; toute assertion sur un rendu local devient alors dépendante du fuseau._
- **Task 10 — une collision de tests, pas un défaut produit.** Ajouter le bouton « Aujourd'hui » a fait échouer un test existant qui cherchait `/aujourd'hui/i` en vue semaine : il attrapait désormais le bouton **en plus** du repère. Assertion resserrée sur `/· aujourd'hui/i`, le point médian distinguant le repère du contrôle. Rappel utile : un matcher de texte large devient ambigu dès qu'on ajoute un libellé voisin.
...
- **+75 tests → 481 verts** ; le bouton « Aujourd'hui » (Task 10) en ajoute 5. Avant lui : **+70 → 476** ; avant la Task 9 : **+50 tests → 456 verts** (54 fichiers), typecheck, oxlint et Prettier propres. Répartition : plages `weekRange`/`monthRange` +10, `group` +12, page `AgendaDetail` +21, hook +4, tuile +3.
...
| --- | --- | --- |
| 2026-07-29 | 1.0 | **Story close.** Preuve device validée par Florian sur l'iPad, navigation temporelle et bouton « Aujourd'hui » compris — dont l'invariant que rien n'automatise : les trois vues à 1024×748 sans scroll. Le pari de la story est tenu : le seam de 10.1 a servi trois plages sans être réécrit. → done. |
| 2026-07-29 | 0.4 | **Bouton « Aujourd'hui » (Task 10), demandé par Florian.** À droite des flèches, il ramène l'ancrage à la date réelle donc à la période **contenant** aujourd'hui — jour, semaine ou mois selon la vue. Sans lui, revenir depuis mars demandait de recompter ses clics ou de quitter la page, ce qui n'est pas évident sur un écran mural. **Jamais `disabled`** quand on y est déjà : un contrôle désactivé est une cible morte que les lecteurs d'écran sautent (précédent consigné en revue de 6.1) ; le clic est simplement idempotent. **Une collision de tests corrigée** : un matcher `/aujourd'hui/i` existant attrapait le nouveau bouton en plus du repère, resserré sur le point médian. **La rangée de contrôle reste à 52px** — le budget du mois est intact. **481 tests verts** (+5), revérifiés sous UTC et Asia/Tokyo. |
