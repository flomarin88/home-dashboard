...
- **⭐ Sortir la barre supérieure du `fixed` (TD-4 pas totalement soldé)** — les tuiles HA (`TopBarSlots` : météo/tortue/poubelle) sont en `fixed` (overlay), donc au **scroll / rebond overscroll iOS** elles restent collées au viewport pendant que l'heure/date + Alarme/Caméras (flux normal) bougent → **décrochage visuel** (retour Florian 2026-07-17). But : que la barre coule avec le reste. **Bloqué par TD-1** : les tuiles ont besoin du provider HA (sous le gate), le chrome (`TopBar`/`Clock`) est au-dessus du gate pour ne pas remonter au reconnect. Pistes : (a) rendre les tuiles dans la rangée `TopBar` via un portail/slot alimenté depuis le provider ; (b) remonter tout le chrome sous le provider en acceptant le remount au reconnect (l'état optimiste vit désormais dans un store module-scope, pas dans le composant → le risque TD-1 d'origine a peut-être baissé) ; (c) garantir zéro scroll (kiosque) + `overscroll-behavior: none` pour que le `fixed` ne décroche jamais. À arbitrer — probablement une story dédiée. [src/ui/TopBarSlots.tsx, src/App.tsx (KioskShell), src/ui/TopBar.tsx]
