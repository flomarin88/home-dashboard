...
- [ ] **Task 0 — ⚠️ PRÉREQUIS HA (Florian, hors app)** — _bloquant pour le device-proof uniquement ; tout le reste est codable et testable avec les mocks `@hakit`._
  - [ ] **`binary_sensor.heures_creuses`** — capteur **template** : `on` quand l'heure courante tombe dans **01:08–06:08** ou **12:38–15:38**, `off` sinon. **Pas d'automation, pas de compteur** — un template qui s'auto-évalue. YAML fourni en Task 6.
  - [ ] **`sensor.hc_hp_prochaine_bascule`** — capteur **template** `device_class: timestamp` : prochaine borne parmi les 4, sinon 01:08 du lendemain. YAML fourni en Task 6.
  - [ ] **Deux helpers `input_number`** : `prix_kwh_creuses` = **0.0890**, `prix_kwh_pleines` = **0.1491** (€/kWh, `step: 0.0001`).
  - [ ] **Ne PAS toucher** au `utility_meter` de 9.1 : il reste le compteur unique (total du jour, graphe, base du coût).
